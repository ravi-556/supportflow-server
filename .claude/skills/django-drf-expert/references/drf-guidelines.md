...
### JWT Authentication (this project's actual auth)

This project uses a custom `JWTAuthentication` (`apps/accounts/authentication.py`), not DRF's built-in `TokenAuthentication` and not `SessionAuthentication` — there's no session cookie, so no CSRF handling is needed (see `security-checklist.md` §3).
...
class JWTAuthentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        if token is None:
            return None  # no token supplied — let AllowAny views through

        try:
            payload = decode_jwt(token)
        except (InvalidTokenError, ExpiredSignatureError):
            return None  # ✅ CRITICAL: return None, never raise here

        user = get_user_from_payload(payload)
        return (user, token)

    def authenticate_header(self, request):
        # ✅ CRITICAL: must be defined, or DRF collapses 401s into 403s
        return 'Bearer'
```

**Rule — this bit for real:** `authenticate()` must return `None` (not raise) on a missing/expired/invalid token, and `authenticate_header()` must be defined. Getting either wrong turns a wrong/missing token into a 403 instead of a 401, or 401s an `AllowAny` view outright — DRF runs authentication before permission checks, so a raised exception here fires before `AllowAny` ever gets a say. A stale token sitting in a browser's `localStorage` broke every public endpoint (KB, CSAT) in this project until this was fixed — don't reintroduce it.

```python
# apps/accounts/agent_views.py — login issues the JWT, doesn't create a DRF Token
...
@permission_classes([AllowAny])
def login(request):
    return Response(authenticate_agent(request.data))  # services.py does the work
```
...
This project versions by literal URL prefix (`/api/v1/agent/...`, `/api/v1/customer/...`), not DRF's `DEFAULT_VERSIONING_CLASS`/`request.version` machinery — there's no `ALLOWED_VERSIONS` setting or version-aware `get_serializer_class()` anywhere in this codebase, and there's no need to introduce one until a real v2 is actually planned.

```python
# urls.py — the version lives in the path, nothing more
urlpatterns = [
...
If a v2 of an endpoint is ever needed, add `path('api/v2/...')` pointing at new views/serializers when that happens — don't pre-build the DRF versioning framework speculatively.
...
✅ Version your API via URL prefix (`/api/v1/...`) from the start
