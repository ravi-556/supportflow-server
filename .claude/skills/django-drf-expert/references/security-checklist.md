...
Django templates auto-escape by default, but a JSON-only API doesn't render templates — the risk moves to whatever consumes the response (the Angular frontend). The API's job is to not launder unsafe data through unrelated channels:

```python
# ❌ DANGEROUS: reflecting raw user input into an admin-rendered HTML page,
# an email template, or any other place that DOES render HTML
send_html_email(f"<p>New ticket: {ticket.subject}</p>")  # subject is user input

# ✅ GOOD: treat user input as data, not markup, everywhere it's reused
send_html_email_from_template('ticket_notification.html', {'subject': ticket.subject})
```

**Rule**: Never use `|safe`/`mark_safe()` on user-generated content in the *few* places Django does render HTML (admin, error pages, emails). The JSON responses themselves aren't a template-injection vector, but don't assume that means the input is safe everywhere else it flows.

### 3. Cross-Site Request Forgery (CSRF) — mostly doesn't apply, but know why

CSRF exploits the browser silently attaching *cookies* to a cross-site request. It only matters when auth state lives in a cookie the browser sends automatically (Django's `SessionAuthentication`).

```python
# settings.py

# ✅ GOOD: stateless Bearer-token/JWT auth in the Authorization header
# is not vulnerable to CSRF — there's no cookie for the browser to
# auto-attach, so csrf_exempt on these endpoints is correct, not risky.
REST_FRAMEWORK = {
...
# ❌ DANGEROUS: only if you ALSO enable SessionAuthentication (e.g. so
# staff can hit the API from a logged-in browser session) — that
# reintroduces the cookie, and CSRF protection must come back with it.
REST_FRAMEWORK = {
    'DEFAULT_AUTHENTICATION_CLASSES': [
        'apps.accounts.authentication.JWTAuthentication',
...
**Rule**: CSRF exemption is safe *because* of Bearer/JWT auth, not despite it — re-evaluate the moment any endpoint accepts cookie-based session auth.
...
✅ CSRF protection enabled on any endpoint using cookie/session auth (not needed for pure Bearer/JWT auth)
