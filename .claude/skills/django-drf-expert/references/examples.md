...
**User Request:**
> "This ticket list endpoint is generating 100+ database queries and it's really slow"
...
class TicketListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ticket
        fields = ['id', 'subject', 'author_name', 'comment_count']

    def get_comment_count(self, obj):
        return obj.messages.count()  # 1 query per ticket!

class TicketListView(APIView):
    def get(self, request):
        tickets = Ticket.objects.all()  # 1 query
        return Response(TicketListSerializer(tickets, many=True).data)

# Serializer causes N+1:
# author_name accesses ticket.author for each ticket  -> 1 query per ticket
# comment_count calls obj.messages.count() per ticket  -> 1 query per ticket
```

**Claude Response:**
```
I can see the N+1 query problem. The view fetches tickets, but the serializer
accesses related `author` (ForeignKey) and counts `messages` (reverse ForeignKey)
for each ticket, causing additional queries.

Solution: Use select_related() for the ForeignKey and annotate() the count
in the queryset, so the serializer just reads already-loaded data.
```
...
# Now the serializer only reads prefetched/annotated data - no additional queries!
```

**Performance Impact:**
- Before: 1 + N (authors) + N (comment counts) = 201 queries for 100 tickets
- After: 2 queries total (tickets + authors via JOIN, comment_count is annotated in the same query)
